...
public:
  Node(DirectX::BoundingBox _bounds, std::vector<const Triangle *> _parentPolys,
       int _depth);
  ~Node(void);
  DirectX::SimpleMath::Vector3 GetCenter() const;
  bool Contains(const DirectX::BoundingBox &_bounds,
                const Triangle &_poly) const;
  bool Intersect(const DirectX::SimpleMath::Ray &_ray, Triangle &_out,
                 float &_dist) const;
};
