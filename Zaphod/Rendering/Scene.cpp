...
Scene::Scene(Camera *_cam, const char *_sceneFile) {
  ...
  std::string sceneFileFolder =
      sceneFileStr.substr(0, sceneFileStr.find_last_of("\\/"));
  ...
  auto ParseCamera = [this](
      const std::unordered_map<std::string, std::string> &values, Camera *cam) {
