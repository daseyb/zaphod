...
BidirectionalPathTracer::Path BidirectionalPathTracer::MakePath(const DirectX::SimpleMath::Ray& _startRay, int _depth, std::default_random_engine& _rnd) const
{
	...
	for (int i = 0; i < _depth; ++i)
	{
		...
		v.BrdfWeight = sample.PDF; // minIntersect.material->F(ray.direction, sample.Direction);
...
float BidirectionalPathTracer::G(const PathVertex &v0, const PathVertex &v1) const
{
...
Color BidirectionalPathTracer::EvalPath(const Path& eye, int nEye, const Path& light, int nLight) const {
	Color L(1, 1, 1, 1);
...
Color BidirectionalPathTracer::IlluminatePoint(Vector3 pos, Vector3 normal, std::default_random_engine & _rnd) const {
	...
	L *= G(PathVertex{ pos, normal }, PathVertex{ lightStart.position, lightStart.direction}) * Le * XM_PI;
