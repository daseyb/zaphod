...
Node::Node(DirectX::BoundingBox _bounds, std::vector<Triangle> _polys, int _depth) {
	m_Bounds = _bounds;
	m_Smallest = false;
  m_Depth = _depth;
	...
	if(m_Bounds.Extents.x * m_Bounds.Extents.y * m_Bounds.Extents.z * 8 > MIN_NODE_SIZE && m_Depth < MAX_DEPTH) {
		Divide();
	} else  {
		m_Smallest = true;		
	}
