...
Node::Node(DirectX::BoundingBox _bounds, std::vector<const Triangle*> _polys, int _depth) {
	m_Bounds = _bounds;
	m_Smallest = false;
	m_Depth = _depth;
	...
	if (m_Bounds.Extents.x * m_Bounds.Extents.y * m_Bounds.Extents.z * 8 > MIN_NODE_SIZE && m_Depth < MAX_DEPTH && _polys.size() > MIN_LEAF_COUNT) {
		Divide();
	}
	else {
		m_Smallest = true;
...
bool Node::Contains(const DirectX::BoundingBox& _bounds, const Triangle& _poly) const {
  return _bounds.Contains(_poly.v(0).Position, _poly.v(1).Position, _poly.v(2).Position);
}
	...
	if (_ray.Intersects(m_Bounds, dist)) {
		...
		if (m_Smallest) {
			...
			for (size_t i = 0; i < m_Polys.size(); i++) {
				tri = *m_Polys[i];
				if (_ray.Intersects(tri.v(0).Position, tri.v(1).Position, tri.v(2).Position, dist) &&
					dist > 0.001f &&
					dist < _outDist) {
					_outTri = tri;
					_outDist = dist;
...
Vector3 Node::GetChildPos(int _index) {
	...
	switch (_index)
	{
	case 0: return Vector3(sizeX, sizeY, sizeZ) + pos; //top	top		top
	case 1: return Vector3(sizeX, sizeY, -sizeZ) + pos; //top	top		bottom
	case 2: return Vector3(sizeX, -sizeY, sizeZ) + pos; //top	bottom	top
	case 3: return Vector3(sizeX, -sizeY, -sizeZ) + pos; //top	bottom	bottom
	case 4: return Vector3(-sizeX, sizeY, sizeZ) + pos; //bottom	top		top
	case 5: return Vector3(-sizeX, sizeY, -sizeZ) + pos; //bottom	top		bottom
	case 6: return Vector3(-sizeX, -sizeY, sizeZ) + pos; //bottom	bottom	top
	case 7: return Vector3(-sizeX, -sizeY, -sizeZ) + pos; //bottom	bottom	bottom
...
void Node::Divide() {
	...
	for (int i = 0; i < 8; i++) {
		...
		for (int j = 0; j < m_Polys.size(); j++) {
			auto poly = *m_Polys[j];
			if (childBounds.Contains(poly.v(0).Position, poly.v(1).Position, poly.v(2).Position)) {
